...
export function mountPlayground(root, { saveSession, notify }) {
  ...
  function persist() {
    ...
    conversation = { ...conversation, title: `${mode === 'live' ? '真实' : '预设'} · ${messages[0]?.content.slice(0, 35) || '学习会话'}`, mode, messages: messages.map(m => ({ role: m.role, content: redactExact(m.content, key) })) };
    saveSession(conversation);
  }
  function lock(value) {
    busy = value;
    for (const element of root.querySelectorAll('input,select,textarea,[data-mode],[data-prompt],#chat-new,#clear-key,#chat-send')) element.disabled = value || (element.dataset.mode === 'live' && !config?.relay);
    $('chat-stop').hidden = !value;
    $('chat-send').textContent = value ? '正在请求…' : mode === 'simulation' ? '运行预设演示 →' : '发送 1 次请求 →';
  }
  function fresh() { messages = []; previousPair = []; conversation = session('学习会话', []); $('playground-error').innerHTML = ''; showMessages(); preview(); }
  ...
  async function send() {
    ...
    if (mode === 'simulation') {
      ...
      messages.push({ role: 'user', content: input.prompt }, { role: 'assistant', content: text });
    ...
    try {
      ...
      messages.push({ role: 'user', content: safePrompt }, { role: 'assistant', content: text }); messages = messages.slice(-40);
      previousPair = input.task === 'explain' && result.text ? [{ role: 'user', content: safePrompt }, { role: 'assistant', content: redactExact(result.text, key) }] : [];
  ...
  $('provider-preset').onchange = () => { const preset = PRESETS.find(p => p.id === $('provider-preset').value); $('api-base').value = preset.baseUrl; $('api-protocol').value = preset.protocol; $('api-model').value = ''; $('api-key').value = ''; $('api-consent').checked = false; preview(); };
  for (const id of ['chat-prompt', 'api-model', 'api-protocol', 'api-limit', 'carry-history']) $(id).oninput = preview;
  $('api-base').oninput = () => { $('api-consent').checked = false; };
