...
**已保存的历史不见了。** 浏览器、隐私模式、主机名和端口都会影响本地存储来源；不同来源不共享 localStorage。可通过 JSON 导入导出手动迁移记录；没有自动跨设备同步。
...
历史搜索、JSON 导入导出和继续会话均在浏览器完成。存储仍为 version 1；消息可带白名单内的 task 标签，旧版无标签消息正常显示。恢复真实会话不恢复密钥、地址、模型或费用授权；未发现本机中继时保持只读，不会降级为伪造的真实模型回复。

历史默认不发送。勾选后仅选择末尾完整且明确标注 explain 的 user/assistant 问答对，每条最多 400 字符，且只用于当前 explain 任务。工具、JSON、轨迹以及缺少任务标签的旧消息不自动进入请求。改变服务、模型或协议会取消历史选择和费用同意，需要重新确认。勾选保存后继续使用原会话 ID；“新会话”开始独立记录。

导入最多 2 MiB、20 个会话、每会话 40 条。发生 ID 冲突时创建副本；容量溢出前说明旧记录淘汰风险。文件内容按文本显示，不执行 HTML、代码或其中声称的工具请求。用户自行粘贴进消息的敏感内容仍需在分享导出前检查。
